						...
						<table id="myTable" class="table table-striped table-borderd">
							...
							<tbody>
								<c:forEach var="message" items="${messages}">
									<tr>
										<th>${message.title}</th>
										<th>${message.user.login}</th>
										<th>${message.date}</th>
										<th><c:url var="url" scope="page"
												value="/panel/viewMessage">
										...
										<th><form:form modelAttribute="message" action="messages"
												method="POST" role="form">
												<input type="hidden" name="id" id="id" path="id"
													tabindex="2" value="${message.id}" />
